...
import { LoanFormValues, LoanTagValues, loanFormSchema } from '@/schemas/loanSchema'
...
import MultiSelectTags, { Tag } from '../tags/MultiSelectTags'

// Updated LoanForm component with UserSelect integration
type LoanFormProps = {
  loan?: Partial<Loan>
  ...
  submitLabel?: string
  addLoanTag: (data: { name: string }) => Promise<LoanTagValues>
}
...
export default function LoanForm({
  ...
  const handleCreateTag = async (name: string) => {
    try {
      const result = await addLoanTag({ name })
      if (result) {
        toast.success('Loan Tag created successfully!')
        return result as Tag
      } else {
        toast.error('Failed to create loan tag.')
        return null
      }
    } catch (error) {
      toast.error('An error occurred while creating the loan tag.')
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  ...
                  <FormControl>
                    <UserSelect
                      onUserSelect={userId => form.setValue('borrowerId', userId)}
                      initialUserId={field.value}
                      includeRole='BORROWER'
                    />
                  </FormControl>
                  <FormDescription>Select a borrower for this loan</FormDescription>
                  <FormMessage />
                </FormItem>
              )}
            />
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  ...
                  <FormControl>
                    <UserSelect
                      onUserSelect={userId => form.setValue('lenderId', userId)}
                      initialUserId={field.value}
                      includeRole='LENDER'
                    />
                  </FormControl>
                  <FormDescription>Select a lender for this loan</FormDescription>
                  <FormMessage />
                </FormItem>
              )}
            />
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  ...
                  <FormControl>
                    <MultiSelectTags
                      tags={(loanTags as Tag[]) || []}
                      selectedTags={(field.value as Tag[]) || []}
                      onChange={tags => form.setValue('loanTags', tags)}
                      onCreateTag={handleCreateTag}
                      placeholder='Select or create tags...'
                    />
                  </FormControl>
                  <FormDescription>Add tags to categorize this loan</FormDescription>
                  <FormMessage />
                </FormItem>
              )}
            />
